#!/usr/bin/env python3
"""
Enhanced Moonriver VoteCast Event Indexer
...
# Event signature for VoteCast(address,uint256,uint8,uint256) - corrected signature
VOTECAST_SIGNATURE = "VoteCast(address,uint256,uint8,uint256)"

# Checkpoint and progress files
CHECKPOINT_FILE = "moonriver_votecast_checkpoint.pkl"
PROGRESS_FILE = "moonriver_votecast_progress.txt"
...
def extract_support_from_raw_data(raw_data: str) -> int:
    """Extract support value from raw_data (bytes 64-95, third 32-byte block)"""
    try:
        if not raw_data or raw_data == "0x":
            return 1  # Default to support if no data
        
        # Remove 0x prefix
        if raw_data.startswith("0x"):
            raw_data = raw_data[2:]
        
        # Ensure we have enough data (at least 96 characters for 3 blocks of 32 bytes each)
        if len(raw_data) < 96:
            return 1  # Default to support if insufficient data
        
        # Extract bytes 64-95 (third 32-byte block) - this is the support value
        support_hex = raw_data[64:96]
        
        # Convert to integer
        support_value = int(support_hex, 16)
        
        # Validate support value (should be 0, 1, or 2)
        if support_value in [0, 1, 2]:
            return support_value
        else:
            return 1  # Default to support if invalid value
            
    except Exception as e:
        print(f"Error extracting support from raw_data: {e}")
        return 1  # Default to support on error
...
def parse_votecast_event(event: Dict) -> Dict:
    ...
    # Extract support from raw data
    support = extract_support_from_raw_data(data)
    ...
    return {
        ...
        "proposal_id": int(topics[2], 16) if len(topics) > 2 and topics[2] != "0x" else 0,
        "support": support,
        "votes": 0,  # Will be extracted from data if needed
        "raw_data": data,
        "raw_topics": json.dumps(topics)
    }

class EnhancedVoteCastIndexer:
    def __init__(self):
    ...
    def initialize_csv(self):
        ...
        self.csv_filename = f"Moonriver_VoteCast_events_incremental_{timestamp}.csv"
    ...
    def run_indexer(self):
        ...
        print("🚀 Starting Enhanced Moonriver VoteCast Event Indexer")
        print("=" * 70)
...
def main():
    indexer = EnhancedVoteCastIndexer()
    indexer.run_indexer()
